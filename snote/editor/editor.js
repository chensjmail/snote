...
editor.prototype = {
	...
	createIframe: function() {
		var _this = this;
		this.ifr = co.append(this.container, 'iframe', {'frameborder': 0, 'style': 'border:0; vertical-align:bottom;width:100%;height:100%', 'class': 'econtent' });
		this.doc =  this.ifr.contentDocument || this.ifr.contentWindow.document; // W3C || IE
		this.doc.designMode = 'on';
		this.doc.open();
		// margin为了消除iframe中的html上部的空白
		this.doc.write('<html><head><style>body{ margin:3px; word-wrap:break-word; word-break: break-all;width:99%;}</style></head><body></body></html>');
